      ...
      case 'mod':
        db.query(`SELECT * FROM custom_evalution_table WHERE ID=${req.query.id}`, (err, data)=>{
          if(err){
            ...
            db.query(`SELECT * FROM custom_evalution_table`, (err, evaluations)=>{
              if(err){
                console.error(err);
  ...
  router.post('/', function (req, res){
    ...
    if(newFileName){
      fs.rename(oldPath, newPath, (err)=>{
        if(err){
          ...
          if(req.body.mod_id){  //修改
              ...
              if(err){
                ...
                fs.unlink('static/upload/'+data[0].src, (err)=>{
                  if(err){
                    ...
                    db.query(`UPDATE custom_evalution_table SET \
            ...
            var title=             //添加
            db.query(`INSERT INTO custom_evalution_table \
      ...
      if(req.body.mod_id){  //修改
        ...
        if(!(req.body.id==null&req.body.f1==null)){               //添加
        db.query(`INSERT INTO custom_evalution_table \
        (title, description, src)
        VALUES('${title}', '${description}', '${newFileName}')`, (err, data)=>{
          if(err){
